...
TopdownScene::TopdownScene(): Scene()
{
    key_handler = new TopdownKeyHandler(this);
    this->Load();

    Game::GetInstance()->SetKeyHandler(key_handler);
}
...
void TopdownScene::Load()
{
    ...
    animation = new Animation(ANIMATION_DEFAULT_TIME);
    ...
    jason = new TopdownJason();
    listObj.push_back(jason);
}
